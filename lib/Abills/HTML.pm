  ...
  #  }
    ...
    foreach my $k (@H) {
      if (ref $attr->{SEL_HASH}->{$k} eq 'ARRAY') {
        ...
        my @sorted_list = ();
        if ($attr->{SORT_VALUE}) {
          @sorted_list = sort {( $attr->{SEL_HASH}->{$k}->{$a} || 0) <=> ($attr->{SEL_HASH}->{$k}->{$b} || 0)
            || $attr->{SEL_HASH}->{$k}->{$a} cmp $attr->{SEL_HASH}->{$k}->{$b}} keys %{$attr->{SEL_HASH}->{$k}};
        }
        else {
          @sorted_list = sort {($a || 0) <=> ($b || 0) || $a cmp $b} keys %{$attr->{SEL_HASH}->{$k}};
        }
